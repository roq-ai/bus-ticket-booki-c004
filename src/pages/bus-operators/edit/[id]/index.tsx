...
import { useRoqClient, useBusOperatorFindFirst } from 'lib/roq';
...
function BusOperatorEditPage() {
  ...
  const { data, error, isLoading, mutate } = useBusOperatorFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: BusOperatorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.bus_operator.update({
        data: values as RoqTypes.bus_operator,
        where: {
          id,
        },
      });
